...
Polazna verzija za dva potpuno odvojena CSV fajla:

1. Loto;
2. Loto Plus.

Svaki CSV zasebno prolazi kroz:

1. wavelet i matched-filter osobine;
2. change-point detection;
3. vremenski linearni autoenkoder;
4. Hidden Markov režime;
5. grafovski model;
6. Bajesovu procenu verovatnoće i neizvesnosti;
7. hronološku walk-forward validaciju;
8. zamrznutu holdout proveru;
9. blok-bootstrap;
10. Monte Karlo nultu hipotezu.

Rezultat su dve NEXT kombinacije:
- jedna za Loto;
- jedna za Loto Plus.

Prvi red svakog CSV fajla smatra se najstarijim.
Poslednji red svakog CSV fajla smatra se najnovijim.
"""
...
def skor_u_verovatnoce(skor: np.ndarray) -> np.ndarray:
    """
    Pretvara kontinuirani skor u 39 marginalnih verovatnoća
...
def change_point_jednog_signala(
    ...
    for presek in range(
        ...
        pre = signal[
            max(0, presek - 80):presek
        ]

        posle = signal[presek:]

        if (
            len(pre) < minimalni_segment
            or len(posle) < minimalni_segment
        ):
            continue

        razlika = float(
            np.mean(posle) - np.mean(pre)
        )
        ...
        jacina = abs(razlika) / max(
            standardna_greska,
            EPS,
        )

        starost = n - presek
        vremenska_tezina = math.exp(
            -starost / 100.0
        )

        ponderisana_jacina = (
            jacina * vremenska_tezina
        )

        if ponderisana_jacina > najbolja_jacina:
            najbolja_jacina = ponderisana_jacina
            najbolji_smer = math.copysign(
                1.0,
                razlika,
            )
...
def vremenski_autoenkoder(
    matrica: np.ndarray,
) -> np.ndarray:
    """
    Linearni vremenski autoenkoder izveden pomoću SVD latentnog
    prostora. Ne koristi buduće redove.
    """

    istorija = matrica[
        -min(
            len(matrica),
            AUTOENKODER_ISTORIJA,
        ):
    ]
    ...
    vremenske_tezine /= (
        vremenske_tezine.sum()
    )

    potpisana = (
        vremenske_tezine @ greska
    )

    intenzitet = (
        vremenske_tezine @ np.abs(greska)
    )
...
def grafovski_model(
    ...
    ponderisana = (
        istorija
        * np.sqrt(vremenske_tezine[:, None])
    )

    susedstvo = (
        ponderisana.T @ ponderisana
    )
    ...
    zbir_reda = susedstvo.sum(
        axis=1,
        keepdims=True,
    )

    prelazi = susedstvo / np.clip(
        zbir_reda,
        EPS,
        None,
    )
...
# =============================================================================
# ZAJEDNIČKO IZRAČUNAVANJE MODELA
# =============================================================================
...
def ispisi_rezultat(
    ...
    if rezultat["statisticki_pouzdan"]:
        print(
            "  DA — pronađena je statistički pouzdana "
            "prediktivna informacija na zamrznutom holdoutu."
        )
        ...
        print(
            "  a Monte Karlo jednostrana p-vrednost "
            "manja je od 0.05."
        )
    else:
        print(
            "  NE — istorijska izvlačenja nisu pokazala "
            "statistički pouzdanu prediktivnu prednost"
        )
...
def main() -> None:
    ...
    if loto["statisticki_pouzdan"]:
        print(
            "  Istorijski podaci pokazali su statistički "
            "pouzdanu prediktivnu informaciju."
        )
    else:
        print(
            "  Istorijski podaci nisu pokazali statistički "
            "pouzdanu prediktivnu prednost iznad 49/39."
        )
    ...
    print()
    print(
        "NEXT kombinacije predstavljaju rezultate šest modela "
        "za prepoznavanje obrazaca."
    )

    print(
        "Rezultat nije dokaz da se buduća poštena izvlačenja "
...
Težine modela:
  Wavelet i matched filter         27.84%
  Change-point detection           24.87%
  Vremenski autoenkoder             7.11%
  Hidden Markov režimi             22.27%
...
Zamrznuta holdout provera:
  Broj koraka:                    160
  Prosečan broj pogodaka:         1.237500
  Slučajno očekivanje:            1.256410
  Brierov skor:                   0.151143
  Najmanje tri pogotka:           6.88%
  Najmanje četiri pogotka:        1.88%
  Najveći broj pogodaka:          4
  Blok-bootstrap 95% interval:    [1.081250, 1.356250]

Monte Karlo nulta hipoteza:
  Broj simulacija:                50,000
  Prosek poštenog modela:         1.256391
  95% opseg poštenog modela:      [1.112500, 1.400000]
  Jednostrana p-vrednost:         0.615368
...
Težine modela:
  Wavelet i matched filter         24.71%
  Change-point detection           30.36%
  Vremenski autoenkoder             7.78%
  Hidden Markov režimi             15.82%
  Grafovski model                   0.00%
  Bajesov model                    21.33%
...
Zamrznuta holdout provera:
  Broj koraka:                    160
  Prosečan broj pogodaka:         1.243750
  Slučajno očekivanje:            1.256410
  Brierov skor:                   0.150672
  Najmanje tri pogotka:           8.12%
  Najmanje četiri pogotka:        0.62%
  Najveći broj pogodaka:          4
  Blok-bootstrap 95% interval:    [1.093750, 1.368750]

Monte Karlo nulta hipoteza:
  Broj simulacija:                50,000
  Prosek poštenog modela:         1.256344
  95% opseg poštenog modela:      [1.112500, 1.400000]
  Jednostrana p-vrednost:         0.582288
...
##############################################################################
Loto:
  Istorijski podaci nisu pokazali statistički pouzdanu prediktivnu prednost iznad 49/39.

Loto Plus:
  Istorijski podaci nisu pokazali statistički pouzdanu prediktivnu prednost iznad 49/39.

NEXT kombinacije predstavljaju rezultate šest modela za prepoznavanje obrazaca.
Rezultat nije dokaz da se buduća poštena izvlačenja mogu pouzdano predvideti.
"""
